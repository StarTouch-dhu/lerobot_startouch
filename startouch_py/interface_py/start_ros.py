...
class MainArmJointPublisher(Node):
    def __init__(self):
        ...
        # 4. 创建关节状态发布者
        self.joint_pub_controller = self.create_publisher(JointState, '/right_arm/joint_states_target', 2)
        self.joint_pub_follower = self.create_publisher(JointState, '/right_arm/joint_states_now', 2)
        print("2话题初始化完成")

        # 5. 初始化并启动重力补偿线程
        self.gravity_thread = threading.Thread(
            target=self.gravity,    # 线程执行的函数
            daemon=True)            # 设为守护线程：主程序退出时线程自动关闭
        self.gravity_thread.start() # 启动线程
        print("重力补偿独立线程已启动")

        self.control_thread = threading.Thread(
            target=self.control,    # 线程执行的函数
            daemon=True)            # 设为守护线程：主程序退出时线程自动关闭
    ...
    def publish_joint_states(self):
        try:
            ...
            joint_msg_controller.position = self.positions_controller  # 关节位置（弧度或角度，取决于硬件）
            ...
            joint_msg_follower.position = self.positions_follower  # 关节位置（弧度或角度，取决于硬件）
    ...
    def control(self):
        while True:
            try:
                self.positions_controller_right = self.arm_controller.get_joint_positions()
                self.positions_follower = self.arm_follower.get_joint_positions()
